...
{init_project_path}. This file must contain both "api_key" and "api_url"."""
    ...
    config_defaults = {
        k: v for k, v in config_defaults.items() if k not in config_dict.keys()
    }
    ...
    if any([x not in insight_panels for x in config_dict["insight_panels"]]):
        ...
        missing_insight_panels = [
            x for x in config_dict["insight_panels"] if x not in insight_panels
        ]
    ...
    if any([x not in config_dict["insight_panels"] for x in insight_panels]):
        print(
            """The following insight panel files are not listed in \
config_file.json:"""
        )
        missing_insight_panels = [
            x for x in insight_panels if x not in config_dict["insight_panels"]
        ]
        print("\n".join(missing_insight_panels))
        print(
            """These will not appear in the dashboard. Please add them \
...
def get_countries(df_map):
    ...
    df_countries = (
        df_countries.groupby(["country_iso", "country_name"]).count().reset_index()
    )
...
def interpolate_colors(colors, n):
    ...
    steps_per_transition = [steps_per_transition + 1] * (n % transitions) + [
    ...
    for i in range(transitions):
        for step in range(steps_per_transition[i]):
            interpolated_rgb = [
                int(
                    rgbs[i][j]
                    + (float(step) / steps_per_transition[i])
                    * (rgbs[i + 1][j] - rgbs[i][j])
                )
                for j in range(3)
            ]
...
def get_map_colorscale(
    df_countries, map_percentile_cutoffs=[10, 20, 30, 40, 50, 60, 70, 80, 90, 99, 100]
):
...
def define_app_layout(fig, buttons, filter_options, map_layout_dict, project_name=None):
    ...
    app_layout = html.Div(
        [
            ...
            dcc.Graph(
                id="world-map", figure=fig, style={"height": "92vh", "margin": "0px"}
            ),
            html.Div(
            ...
            html.Div(
                ...
                [
                    dbc.Button(
                        "Report", id="report-button", style={"text-align": "center"}
                    )
                ]
                + [
...
def get_visuals(
    buttons, insight_panels, df_map, df_forms_dict, dictionary, quality_report, filepath
):
...
def create_modal(visuals, button, filter_options):
    if visuals is None:
        ...
        insight_children = [
            dbc.Tabs(
                [
                    dbc.Tab(
                        dbc.Row([dbc.Col(dcc.Graph(figure=figure), id=id)]), label=label
                    )
        ...
        about_list += [
            "<strong>" + label + "</strong>" + about for _, _, label, about in visuals
        ]
        about_str = "\n".join(about_list)
    ...
    modal = [
            ...
            [
                dbc.Accordion(
                    [
                        dbc.AccordionItem(
                            title="Filters and Controls",
                            children=[define_filters_controls_modal(**filter_options)],
                        ),
        ...
        define_footer_modal(
            generate_html_text(instructions_str), generate_html_text(about_str)
        ),
    ]
    return modal
...
def define_filters_controls_modal(
    sex_options, age_options, country_options, outcome_options, add_row=None
):
    row = dbc.Row(
        [
            dbc.Col(
...
def register_callbacks(
    ...
    def update_country_display(selected_values, all_options):
        ...
        selected_labels = [
            value_label_map[val] for val in selected_values if val in value_label_map
        ]
    ...
    def update_figures(click, button, genders, age_range, outcomes, countries):
        df_map["filters_age"] = df_map["filters_age"].astype(float)
        df_map_filtered = df_map[
            (
                (df_map["filters_sex"].isin(genders))
        ...
        for key in df_forms_filtered.keys():
            ...
            df_filtered["filters_age"] = df_filtered["filters_age"].astype(float)
...
def main():
    ...
    if get_data_from_api is False:
        try:
            ...
            df_map = pd.read_csv(os.path.join(vertex_dataframes_path, "df_map.csv"))
            ...
            df_forms_dict = {
                k.split(".csv")[0]: pd.read_csv(os.path.join(vertex_dataframes_path, k))
    ...
    app.layout = define_app_layout(
        fig, buttons, filter_options, map_layout_dict, config_dict["project_name"]
    )
    ...
    register_callbacks(
        ...
        filter_options,
        init_project_path,
        config_dict["save_filtered_public_outputs"],
    )

    if config_dict["save_public_outputs"]:
        public_path = os.path.join(init_project_path, config_dict["public_path"])
        ...
        os.makedirs(
            os.path.dirname(os.path.join(public_path, "data", "")), exist_ok=True
        )
        ...
        metadata_file = os.path.join(public_path, "data/dashboard_metadata.txt")
        ...
        config_json_file = os.path.join(public_path, "public_config_file.json")
